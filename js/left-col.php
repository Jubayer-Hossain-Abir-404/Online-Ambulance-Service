...
<?PHP 
	...
	$q='SELECT DISTINCT `receiver_name`, `sender_name` FROM `messages` WHERE `sender_name`="'.$_SESSION['user_name'].'" OR `receiver_name`= "'.$_SESSION['user_name'].'" ORDER BY `date_time` DESC';
	...
	if($r){
		if(mysqli_num_rows($r)>0){
			...
			while($row=mysqli_fetch_assoc($r)){
				...
				}elseif($_SESSION['user_name']==$receiver_name){
					//add the sender_name but only once
					//so to do that check the user in array
					if(in_array($sender_name, $added_user)){
						//dont add sender_name_name because
						//he is already added
					}else{
						//add the sender_name
						?>
